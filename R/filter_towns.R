...
filter_towns <- function(df.combined, catalog_name="ANY", town="ANY") {
  ...
  if (catalog_name != "ANY") {
    publications <- df.combined %>% filter(catalog == catalog_name)
    publications <- publications %>% filter(publication_place==town)
  } else {
    publications <- df.combined %>% filter(publication_place==town)
  }
