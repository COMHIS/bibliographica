get_relation_keyword <- function(x, full_name, languages=c("english")) {
  ...
  if ("finnish" %in% languages) {
    f[i] <- "inst/extdata/fi_relation_keywords.csv"
    i <- i + 1
  }
  if ("swedish" %in% languages) {
    f[i] <- "inst/extdata/sv_relation_keywords.csv"
    i <- i + 1
  }
  if ("english" %in% languages) {
    f[i] <- "inst/extdata/en_relation_keywords.csv"
    i <- i + 1
  }
  if ("latin" %in% languages) {
    f[i] <- "inst/extdata/la_relation_keywords.csv"
    i <- i + 1
  }
