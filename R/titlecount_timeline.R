...
titlecount_timeline <- function (x, group, nmin = 0, mode = "n") {
  ...
  df2 <- x %>% filter(!is.na(group)) %>% group_by(publication_decade, group) %>%
     summarize(n = n())

  # Remove entries with too few occurrences
  df2 <- df2 %>% filter(!is.na(publication_decade) &
    group %in% setdiff(names(which(table(df2$group) >= nmin)), "NA"))
  ...
  p <- ggplot(dfs, aes(y = mode, x = publication_decade, shape = group, linetype = group)) +
     geom_point(size = 4) +
