...
## Document size comparisons
  ...
  * Some dimension info is provided in the original raw data for altogether `r sum(!is.na(df$physical_dimension))` documents (`r round(100*sum(!is.na(df$physical_dimension)),1)`%) but could not be interpreted for `r sum(!is.na(df$physical_dimension) & (df$gatherings == "NA"))` documents (ie. dimension info was successfully estimated for `r round(100 - 100 * sum(!is.na(df$physical_dimension) & (df$gatherings == "NA"))/sum(!is.na(df$physical_dimension)), 1)` % of the documents where this field was not empty).
