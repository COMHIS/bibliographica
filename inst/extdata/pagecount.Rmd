...
Note: there are `r length(which(is.na(df$area) & (!is.na(df$gatherings.original) | !is.na(df$width.original) | !is.na(df$height.original))))` documents that have some dimension info but sheet area information could not be calculated for some reason. 
