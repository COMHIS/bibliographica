...
df2 <- df %>% group_by(publication_decade, gatherings) %>%
              summarize(paper = sum(paper, na.rm = TRUE), n = n()) %>%
	      filter(df2, gatherings %in% names(which(table(df2$gatherings) >= 50))) %>%
	      filter(!is.na(gatherings))

p <- ggplot(df2, aes(x = publication_decade, fill = gatherings)) +
                 geom_bar(position = "stack", stat = "bin") +
		 ggtitle("Gatherings counts over time")
print(p)

p <- ggplot(df2, aes(x = publication_decade, fill = gatherings)) +
       geom_bar(position = "fill") +
       ggtitle("Gatherings proportions over time")
print(p)    
```

```{r paperconsumption2, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=13, fig.height=6}
df <- df.preprocessed
df2 <- df %>% group_by(publication_decade) %>%
              summarize(paper = sum(paper, na.rm = TRUE), n = n()) 
