...
#if (exists("df.gatherings")) {
#  df0 <- df <- left_join(df.preprocessed, df.gatherings, by = "id")
#} else {
#  df0 <- df <- df.preprocessed
#}
```
...
#mean.pagecounts <- get_mean_pagecounts(df0, exclude.plates = TRUE)
mean.pagecounts <- get_mean_pagecounts(df.combined, exclude.plates = TRUE)
```
...
message(paste0(colnames(dfs), sep=" "))
...
#df <- df0
  ...
  #dff <- filter(df, volcount == 1 & is.na(volnumber))
	dff <- filter(df.combined, volcount == 1 & is.na(volnumber))
...
names(dfg) <- c("pagecount", "gatherings", "documents")
dfg$gatherings <- factor(dfg$gatherings, levels = levels(df$gatherings))
ylims <- range(dfg$pagecount)
p <- ggplot(dfg, aes(x = gatherings, y = pagecount)) 
n <- 1 + nchar(max(na.omit(table(dfg$pagecount))))
ylim <- ylim(ylims)
p <- p + scale_y_log10(breaks=10^(0:n))
p <- p + geom_point(aes(size = documents))
p <- p + scale_size(trans="log10")
p <- p + ggtitle(paste("Estimated and original pagecount\n(n=", sum(dfg$documents), ")", sep = ""))
p <- p + xlab("Size (gatherings)")
p <- p + ylab("Pagecount (original and estimated)")
p <- p + coord_flip()
print(p)

dfs <- subset(df, is.na(pagecount.orig) & !is.na(pagecount))
p <- ggplot(dfs,
...
df2$na[df2$na == 0] <- NA
df2 <- df2[-grep("long", df2$gatherings),]
df2$gatherings <- droplevels(df2$gatherings)
df2$gatherings <- map_gatherings(df2$gatherings)
library(ggplot2)
...
p <- p + guides(size = guide_legend(title = "N"))
print(p)

theme_set(theme_bw(25))
library(dplyr)
topg <- names(top(df$gatherings, n = 4))
df2 <- df %>% filter(gatherings %in% topg) %>%
              group_by(publication_decade, gatherings) %>%
              summarize(na = sum(is.na(pagecount.orig) |
	                         (!pagecount.orig == pagecount)))
# df2$na[df2$na == 0] <- NA
# df2 <- df2[-grep("long", df2$gatherings),]
# df2 <- df2[-grep("NA", df2$gatherings),]
df2$gatherings <- droplevels(df2$gatherings)
df2$gatherings <- map_gatherings(df2$gatherings)
library(ggplot2)
p <- ggplot(df2, aes(x = publication_decade, y = gatherings, size = na))
p <- p + geom_point()
p <- p + xlim(c(min(na.omit(df$publication_decade)), 1800))
#p <- p + ggtitle(paste("Missing page count (original; n=", sum(na.omit(df2$na)), ")", sep = ""))
# p <- p + ggtitle("Missing page count")
p <- p + xlab("Publication decade")
# p <- p + ylab("Document dimension (estimated)")
p <- p + ylab("")
p <- p + guides(size = guide_legend(title = "N"))
pdf("~/tmp/missingpages.pdf", width=7, height=7)
print(p)
dev.off()
...
for (g in c("1to", "2fo", "4to", "8vo")) {
  ...
  p <- ggplot(df3, aes(x = publication_year, y = pagecount.orig)) + geom_point(aes(size = n))
  p <- p + geom_smooth(data = df2, aes(x = publication_year, y = pagecount.orig))
  p <- p + ggtitle(paste(g, "pagecount over time (books with over", minpages, "pagecount)"))
  print(p)
}
```
