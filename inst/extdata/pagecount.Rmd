...
df2 <- df2 %>% filter(gatherings %in% top) %>%
	       filter(!is.na(gatherings) & !gatherings == "NA")

p <- ggplot(df2, aes(x = publication_decade, y = paper, fill = gatherings)) +
                 geom_bar(position = "stack", stat = "identity") +
		 ggtitle("Gatherings paper consumption over time")
print(p)

p <- ggplot(df2, aes(x = publication_decade, y = n, fill = gatherings)) +
       geom_bar(position = "fill", stat = "identity") +
       ggtitle("Gatherings count proportions over time") +
