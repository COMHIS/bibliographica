...
## Page counts
  ...
  * Page count available for `r sum(!is.na(df$pagecount.orig))` documents (`r round(100*(sum(!is.na(df$pagecount.orig)))/nrow(df), 1)`%). 
  ...
  * Page count available for total `r sum(!is.na(df$pagecount))` documents (`r round(100*(sum(!is.na(df$pagecount)))/nrow(df), 1)`%). This includes both immediately available and estimated page counts.
