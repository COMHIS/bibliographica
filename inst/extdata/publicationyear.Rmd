...
Publication frequency information is available for `r sum(!is.na(df$publication_frequency_text))` documents (`r round(100*mean(!is.na(df$publication_frequency_text)))`%). The links are invalid if the lists are empty. The annual frequencies are converted to plain text according to their closest match in [this table]().
