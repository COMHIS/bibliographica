...
test_that("page count is correct", {
  ...
  expect_equal(sum(polish_pages("[4],56,65-176;45,[1]p.")$estimated.pages[[1]]), 180)
  expect_equal(sum(polish_pages("[4],56,65-176;45,[1]p.")$total.pages), 226)  
  ...
  expect_equal(sum(polish_pages("[2],iii,[1],25,[26],50-69,[1];213,[67]p.,plates")$total.pages), 386)
  ...
  expect_equal(sum(polish_pages("[14],356,361-408;76,79-216,[14]p.,plates")$total.pages), 656)
  ...
  expect_equal(sum(polish_pages("[2],99,[4],102-110,113-119,[1],119-120,[2];[2],121-246p.,tables")$total.pages), 261)
